...
import { LayoutDashboard, Users, Store, ShoppingBag, LogOut, Menu, Shield, Moon, Sun } from 'lucide-react';
  ...
  return (
    <div className="flex h-full flex-col bg-slate-900 text-white">
      <div className="flex h-16 items-center gap-2 border-b border-slate-700 px-5">
        ...
        <span className="ml-1 rounded bg-violet-600 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide">
          Admin
        </span>
      </div>
      ...
      <div className="border-t border-slate-700 p-4">
        <div className="mb-3 flex items-center gap-3 px-1">
          <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-violet-600 text-sm font-semibold">
    ...
    <div className="flex h-screen overflow-hidden bg-slate-50 dark:bg-slate-800">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <header className="flex h-16 items-center justify-between border-b border-slate-200 bg-white px-4 dark:border-slate-700 dark:bg-slate-900 lg:hidden">
          ...
          <span className="font-semibold text-slate-800 dark:text-white">MVEP Admin</span>
          <button
            onClick={toggle}
            aria-label={isDark ? 'Switch to light mode' : 'Switch to dark mode'}
            className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800"
